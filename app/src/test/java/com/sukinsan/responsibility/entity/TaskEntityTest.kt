...
class TaskEntityTest {
    ...
    fun success_create_entity() {
        val date = Date(0)
        val task = TaskEntity("task id", 1, RemindRuleEnum.DAILY, "Drink water", date)
        ...
        Assert.assertEquals(1, task.notificationId)

        Assert.assertEquals(
            "TaskEntity(id='task id', notificationId=1, remindRule=DAILY, description='Drink water', createdAt=Thu Jan 01 02:00:00 EET 1970, doneAt=null, failedAt=null, workerManagerId=null)",
            task.toString()
        )

        Assert.assertEquals(
            TaskEntity("task id", 1, RemindRuleEnum.DAILY, "Drink water", date, null, null),
            task
        )
    }
    ...
    fun success_json_serialization_deserialization() {
        val task = TaskEntity(
            "task id", 1, RemindRuleEnum.DAILY, "Drink water",
        ...
        Assert.assertEquals(
            "{\"id\":\"task id\",\"notificationId\":1,\"remindRule\":\"DAILY\",\"description\":\"Drink water\",\"createdAt\":\"Nov 10, 3000 9:10:00 PM\",\"doneAt\":\"Nov 10, 3000 10:25:00 PM\",\"failedAt\":\"Nov 10, 3000 11:30:00 PM\",\"workerManagerId\":\"UUID-UUID\"}",
            task.toJson()
        )
